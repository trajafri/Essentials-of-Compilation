...
(Section~\ref{sec:ast}). We use grammars to define programming languages
...
Recall that a \emph{symbolic expression} (S-expression) is either
\begin{enumerate}
\item an atom, or
...
%
The name $\Int$ is also a non-terminal, however,
...
this corresponds to integers represented with 64-bits, i.e., in the range
$-2^{63}$ to $2^{63}-1$.
%
...
corresponds to one grammar rule and the body of each clause makes a
